    ...
    @preconcurrency CBPeripheralDelegate
{
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        if DeviceStore.shared.isNameBlocked(name: deviceName) { return }
        ...
        DeviceStore.shared.noteBLEPresence(name: deviceName)
        ...
        if let data = manufacturerData, data.count > 0 {
            if data[0] != 76 {
                ...
                if readBLEDevice &&
                    ...
                    if let device = DeviceStore.shared.getByName(deviceName) {
                ...
                if [16, 12].contains(data[2]) &&
                    ...
                    activePolicy == .allow &&
                    isConnectable {
                    if let device = DeviceStore.shared.getByName(deviceName), device.deviceModel != nil {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if characteristic.uuid == CBUUID(string: "2A24") {
            if let data = characteristic.value, let model = data.ascii(), let deviceName = peripheral.name, let vendor = bleDevicesVendor[deviceName] {
                ...
                if var device = DeviceStore.shared.getByName(deviceName), device.deviceModel != model{
                    if vendor == "Apple Inc." {
    ...
    func getAirpods(peripheral: CBPeripheral, data: Data, messageType: String) {
        ...
        if DeviceStore.shared.isNameBlocked(name: name) { return }
        ...
        if let deviceName = peripheral.name{
            ...
            if let Case = DeviceStore.shared.getByName(deviceName + " (Case)".local) { model = Case.deviceModel ?? model }
            ...
            if !["Airpods Max", "Beats Solo Pro", "Beats Solo 3", "Beats Studio Pro"].contains(model) {
                if caseLevel != 255 { DeviceStore.shared.update(Device(deviceID: deviceID, deviceType: "ap_case", deviceName: deviceName + " (Case)".local, deviceModel: model, batteryLevel: Int(caseLevel), isCharging: caseCharging, lastUpdate: now)) }
                ...
                if model == "Beats Studio Pro" {
                    DeviceStore.shared.update(Device(deviceID: deviceID, deviceType: "ap_case", deviceName: deviceName, deviceModel: model, batteryLevel: Int(rightLevel), isCharging: rightCharging, lastUpdate: now))
                } else {
                    leftLevel = leftLevel != 255 ? leftLevel : 0
                    rightLevel = rightLevel != 255 ? rightLevel : 0
                    DeviceStore.shared.update(Device(deviceID: deviceID, deviceType: "ap_case", deviceName: deviceName, deviceModel: model, batteryLevel: Int(max(rightLevel, leftLevel)), isCharging: rightCharging + leftCharging > 0 ? 1 : 0, lastUpdate: now))
                }
            }
        }
    }
