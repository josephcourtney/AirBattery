...
final class MultipeerService: ObservableObject {
    ...
    init(serviceType: String, deviceStore: DeviceStore) {
        self.deviceStore = deviceStore
        self.localDeviceID = getMacDeviceUUID()
        ...
        transceiver.receive(Data.self) { [weak self] data, peer in
            ...
            Task { @MainActor [weak self] in
                ...
                case "resend":
                    var allDevices = self.deviceStore.getAll()
                    ...
                    do {
                        ...
                        let message = NCMessage(id: String(self.nearcastGroupID), sender: self.localDeviceID ?? self.deviceName, command: "", content: data)
                ...
                case "":
                    ...
                    if let jsonString = decryptNearcastString(message.content, groupID: self.nearcastGroupID, sharingKey: self.nearcastSharingKey) {
                        if let jsonData = jsonString.data(using: .utf8) {
                            let url = BatterySnapshotStore.nearcastDirectory.appendingPathComponent("\(message.sender).json")
    ...
    func refeshAll() {
        ...
        let message = NCMessage(id: String(nearcastGroupID), sender: self.localDeviceID ?? self.deviceName, command: "resend", content: "")
    ...
    func createInfo(type: Int = 0, title: String, info: String, atta: String = "") -> NCMessage? {
        do {
            ...
            return NCMessage(id: String(self.nearcastGroupID), sender: self.localDeviceID ?? self.deviceName, command: "notify", content: data)
        } catch {
            print("Write JSON error：\(error)")
        }
        return nil
