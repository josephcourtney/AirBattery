...
struct popover: View {
    ...
    @State private var hiddenDevices = DeviceStore.shared.hiddenDevices()
    ...
    @State private var allNearcast = getFiles(withExtension: "json", in: BatterySnapshotStore.nearcastDirectory)
    ...
    private func hasFollowingVisibleRow(after index: Int) -> Bool {
        ...
        return allDevices[(index + 1)...].contains {
            !AirPodsPresentation.isSecondaryRow($0, in: allDevices)
        }
    }
    ...
    private func hideAirPodsGroup(_ group: AirPodsBatteryGroup) {
        ...
        hiddenDevices = DeviceStore.shared.hiddenDevices()
    ...
    private func airPodsPresentation(
        ...
        AirPodsPresentation.logicalPresentations(
    ...
    var body: some View {
        ZStack{
            ...
            VStack(spacing: 0){
                PopoverToolbarSurfaceContent(
                    ...
                    onRefreshNearcast: {
                        AppEnvironment.shared.nearcast.refeshAll()
                        if fromDock {
                            DockPopoverController.shared.hide()
                        } else {
                            DispatchQueue.main.asyncAfter(
                                ...
                                allDevices = DeviceStore.shared.getAll()
        ...
        .onReceive(monitoring.$secondTick) { _ in
            if !fromDock && StatusBarController.shared.isMenuOpen {
                ...
                if nearCast { allNearcast = getFiles(withExtension: "json", in: BatterySnapshotStore.nearcastDirectory) }
            }
        }
    }
}
